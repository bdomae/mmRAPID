...
%         SNR = SNR_options(snr_index);
    ...
    for dd=1:traing_size
        best_DFT_bin = label_DFT_based_quan(dd);
    ...
    for m_index = 1:length(M_options)
        ...
        label_DFT_based_GT = label_DFT_based(traing_size+1:end);
        test_data_size = data_size - traing_size;
        y_DFT_nc_test = y_DFT_nc_quan(:,traing_size+1:end);
        y_DFT_nc_GT = y_DFT_nc(:,traing_size+1:end);
...
%         gain_loss = 10*log10(Nr^2)*ones(1,data_size) - 10*log10(abs(sum(exp(1j*pi*(0:Nr-1).'*sin(AoA - AoA_est)),1)).^2);
        for dd = 1:test_data_size
            gain_loss(dd) = y_DFT_nc_GT(label_DFT_based_GT(dd),dd) - y_DFT_nc_test(max_corr_indices(dd),dd);
        end
